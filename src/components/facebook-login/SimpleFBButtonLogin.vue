<template>
  <span>
    <slot :login="login">
      <q-btn outline no-caps align="left" class="full-width" color="indigo-10" @click="login">
          <img
...
export default {
  ...
  methods: {
    ...
    saveProfileData (res, response) {
      if (!response || response.error) {
        this.$emit('error', response.error)
      } else {
        this.profileDataFB = response
        this.$emit('conected', response)
    ...
    handleResponseLoginFB (res) {
      this.getLoginStatusFB(res)
      if (res.status === 'connected') {
        window.FB.api(
          ...
          response => this.saveProfileData(res, response)
        )
      }
    },
    login () {
